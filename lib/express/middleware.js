...
* @param {import('express').NextFunction} next
*/
export function requireNoLogin(request, response, next) {
	if (response.locals.authenticated) {
 ...
 */
export function createGetUser(altGetUserFromRequest) {
	return altGetUserFromRequest ? [conditionalMiddleware(altGetUser(altGetUserFromRequest), session)] : session;
}
...
export const tokenSwap = (request, response, next) => {
	...
	return getCookieFromToken(url.searchParams.get('token')).then(maybeCookie => {
		...
		if (!config.safe.acl.domains.includes(url.hostname)) {
			console.warn(`Token swap performed on domain not part of ACL: ${url.hostname} (for ${url.href})`);
		}
