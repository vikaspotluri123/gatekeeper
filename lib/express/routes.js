...
export async function setupRoutes(app, altGetUserFromRequest) {
	...
	app.get('/api/v1/rest/:url(*)', getUser, requireLogin, apiV1.validUrl, apiV1.validateAccess, noAccess);
	...
	app.get('/login', requireNoLogin, auth.outgoing);
	app.get('/login/callback', requireNoLogin, auth.incoming, apiV1.sendToken, auth.redirect);
